...
class MinHashing:
    ...
    def compute_for_doc(self, shingles: Set[int], hash_params: List[Tuple[int, int]], prime: int) -> List[int]:
        """Compute MinHash signature for one document using NumPy vectorized operations."""
        if not shingles:
            return [prime] * self.num_perm

        # ✅ Fix: reduce all shingle values modulo prime to avoid OverflowError
        ...
        # (a*x + b) % prime for all hash functions and shingles at once
    ...
    def compute_signatures_rdd(self, shingled_rdd: RDD) -> RDD:
        """Compute MinHash signatures for each document in parallel using Spark."""
        broadcast_hash_params = shingled_rdd.context.broadcast(self.hash_params)
        prime = self.prime

        # Each Spark worker computes MinHash for its assigned document subset
        return shingled_rdd.mapValues(
            lambda shingles: self.compute_for_doc(shingles, broadcast_hash_params.value, prime)
        )
